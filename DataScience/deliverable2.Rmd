...
standings <- standings %>%
  rename(rank=Rk, team=Tm, games=G, win=W, losses=L, run_diff=Rdiff)
...
Specifically the matchup is formatted like `Atlanta Braves vs New York Mets`. We want to offer consistency with our other data sets, so we are going to split this `matchup` into `home` and `away`. We'll need to make a lookup table though to convert the full team name into it's 3 character abbreviation. I just created my own csv file in an editor, so we'll have to load that.
```{r}
